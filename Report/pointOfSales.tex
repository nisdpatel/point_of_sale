...
%\setlist[description]{leftmargin=4cm}
% *** CITATION PACKAGES ***
%
...
\author{Ricky Hempel, Nisha Patel, and Josh Cannon\\% <-this % stops a space
}
...
\begin{abstract}
A point sale system is an application for store, in general. It has a functionality of purchasing  various products from different departments by adding them into the cart and checks out the cart by paying the total amount. The target audience for this project is anyone who uses a checkout. 
\end{abstract}
...
%\subsubsection{What might get added later: }
...
\hline \hline
1 &Click on different menus to see all products that are being offered in the store & Shopper & Med & 1\\
\hline
2 & Add item to cart & Shopper & Med & 1\\
\hline
3 & Checkout & Shopper and Cashier (applicable to both) & Med & 2\\
\hline
\end{tabular}}\\
...
\end{enumerate} 
\textbf{User Stories}:\\
User Story 1: As a user, he or she will be able to click on different department to see what products are being offered\\
User Story 2: Once the user has explore the menu, now he or she can add items in to the cart by clicking ``add to cart" button. Furthermore, the user can see the current running total of the cart on the display. \\
User Story 3: When the user is ready to checkout, he or she can click``checkout" button on the screen and that will allow them to pay the final price including the taxes.
...
We used built-in observable pattern that can maintain a list of object's dependents and we decided to use this pattern because observer pattern will notifies its dependent objects whenever a subject is modified.  Furthermore, we are using Builder design patter because it separate the construction of a complex objects from its representation so the same construction process can create different representations. In our POS, we are using this method to separate "manager" and "employee" view. For example, on the manager window's, they should be able to view the inventory list where they can add/remove items into the database on the fly. However, on the employee window, they can only have limited access. For instance, employee cannot view the inventory list and they do not have access to add/remove items into the database. We used Microsoft SQL database to store out products according to there category. We also used entity framework that offers an automated mechanism to developers for storing and accessing data in the database. 


\section{Results}
%This section will start out a little vague, but it should grow as your project evolves.  With each deliverable you hand in, give me a final summary of where your project stands.  By the end, this should be a reflective section discussing how many of your original goals you managed to attain/how many desired use cases you implemented/how many extra features you added. 
Our point of sales can do the following tasks: First, user can view the menu-driven interface for all departments with the list of items, prices, and products description. Second, a user can select a department and view the offered items. Third, a user can select item(s) and add into the cart by clicking "Add to Cart" button. Fourth, it can keep track of added or removed items from the cart (constantly updating the cart whenever a user makes any changes). Fifth, when a user click on "checkout" button, it will help them the current running total with the taxes. \\
...
For the future development, we have to work on the security issues by create a login page for the user and it will grant access according to the entered credentials. For instance, admin has full access to the system whereas employees has limited access. Next, we want to create a prettier interface by adding additional functionality such as add images of the products, so user know what they are buying. Then, create a finite list of inventory with the item counts, so the manager can see how many items has been sold and restock them as needed. Last, we want to add a functionality where user can select a payment type (cash or credit) and process the payment.
We almost met the expected goals. We decided to shift gears with the initial design because changing a window every time when a user clicks was bit challenging, so we decided to go with a simpler approach by using a listbox. We will be selling this software directly to the retailer once it has all the functionality and we will be linking on your resume. 
